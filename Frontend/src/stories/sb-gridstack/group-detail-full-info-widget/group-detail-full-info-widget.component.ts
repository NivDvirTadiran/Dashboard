...
export class GroupDetailFullInfoWidgetComponent extends BaseWidget implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.registerGsWidget(this.widget)
      .subscribe(data => {
        console.log(`Widget ${this.widget.id} of type ${GROUP_DETAIL_FULL_INFO_WIDGET_ID} added and received data/state:`, data);
      });
  }
  ...
  public fetchWidgetData(config: WidgetConfig): Observable<GroupDetailInfoDataItemType[]> {
    ...
    if (!groupId) {
      ...
      return this.emisSoapService.getGroupDetailFullInfo(groupId).pipe(
        map(data => {
          console.log(`Fetched group-detail-full-info-table:`, data);
          return data || [];
        }),
        catchError(err => {
