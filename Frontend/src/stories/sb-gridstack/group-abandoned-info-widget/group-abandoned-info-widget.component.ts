...
import {EmisSoapService, BlockItemType} from "src/app/services/emis-soap.service";
...
export class GroupAbandonedInfoWidgetComponent extends BaseWidget implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.registerGsWidget(this.widget)
      .subscribe(data => {
        console.log(`Widget ${this.widget.id} of type ${GROUP_ABANDONED_INFO_WIDGET_ID} added and received data/state:`, data);
      });
  }
  ...
  public fetchWidgetData(config: WidgetConfig): Observable<BlockItemType[]> {
    ...
    return this.emisSoapService.getGroupAbandonedInfo(groupId).pipe(
      map(data => {
        console.log(`Fetched group-abandoned-info-table:`, data);
        return data.returnArrayOfBlocks || [];
      }),
      catchError(err => {
