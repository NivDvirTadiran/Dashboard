...
export class GroupAgentsInfoWidgetComponent extends BaseWidget implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.registerGsWidget(this.widget)
      .subscribe(data => {
        console.log(`Widget ${this.widget.id} of type ${GROUP_AGENTS_INFO_WIDGET_ID} added and received data/state:`, data);
      });
  }
  ...
  public fetchWidgetData(config: WidgetConfig): Observable<DataItemType[]> {
    ...
    return this.emisSoapService.getGroupAgentsInfo(groupId).pipe(
      map(data => {
