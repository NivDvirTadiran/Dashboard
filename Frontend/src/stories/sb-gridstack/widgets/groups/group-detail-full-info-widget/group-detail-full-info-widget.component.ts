...
@Component({
  selector: 'group-detail-full-info-widget',
  templateUrl: './group-detail-full-info-widget.component.html',
  styleUrls: ['./group-detail-full-info-widget.component.scss'],
  standalone: true,
  imports: [CommonModule, GSBaseWidget, GsGenericTableComponent]
})
export class GroupDetailFullInfoWidgetComponent extends GSBaseWidget implements OnDestroy {
  ...
  constructor(
    ...
    super(widgetManager);
    console.log('constructor GroupDetailFullInfoWidgetComponent');
  }
  ...
  public override fetchData(): void {
    if (this.widget && this.widget.state) {
      ...
      if (!groupId) {
        console.error('Group ID is required for group-detail-full-info-table widget');
        this.widget.state.error = 'Group ID is required';
        this.widget.state.loading = false;
        return;
      }

      this.emisSoapService.getGroupDetailFullInfo(groupId).pipe(
        map((response: GroupDetailInfoDataItemType[]) => { // Changed data to response
          console.log(`Fetched group-detail-full-info-table:`, response);
          // The data is nested within the first element's returnArray
        ...
        catchError(err => {
          console.error('Error fetching group-detail-full-info-table data in manager:', err);
