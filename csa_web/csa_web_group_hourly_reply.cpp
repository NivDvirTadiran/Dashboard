...
//------------------------------------------------------------------------------------------------
static BswLog_c csa_web_group_hourly_reply_log_d(BswMakeLogSite(FC_CSA_WEB, 1),LM_INTER_CSCI, LOGL_DETAILED);
static BswLog_c csa_web_group_hourly_reply_log_i(BswMakeLogSite(FC_CSA_WEB, 1),LM_INTER_CSCI, LOGL_INFO);


//------------------------------------------------------------------------------------------------
void GroupHourlyReply::AddNewGroupBlock(CsaWebServer *soap,unsigned int groupId, ns1__ReturnMatrixDataTypePerGroup &ReturnData)
{   
...
void GroupHourlyReply::CreateNewRow(CsaWebServer *soap)
{
  CurrentRow = soap_new_ns1__DataItemRow(soap, 1);
}
//------------------------------------------------------------------------------------------------

void GroupHourlyReply::InsertFullDataBG(CsaWebServer *soap,unsigned int *bgcolor_ptr, std::string ersid, std::string ersname, std::string value)
{
  ns1__DataItemType* tmp;

  csa_web_group_hourly_reply_log_d << "GroupHourlyReply::InsertFullDataBG"
                                   << "\ners_id   - " << ersid.c_str()
...
//------------------------------------------------------------------------------------------------
// For Array of Matrix
void GroupHourlyReply::InsertRowToMatrix(CsaWebServer *soap,ns1__returnDataRowsPerGroup* matrix)
{
...
//--------------------------------------------------------------------------------------------
// Not Used - Example
void GroupHourlyReply::InsertDataToRow(CsaWebServer *soap,unsigned int groupId)
{
...
RetCode_t GroupHourlyReply::InsertDataToRows(CsaWebServer*                      soap,
                                             ...
                                             unsigned int                       blockInd)
{
  ...
  csa_web_group_hourly_reply_log_d << "GroupHourlyReply::InsertDataToRows <" << groupId << ">" << LOG_TERMINATOR;
  ...
  if ( (rc == BSWRC_OK) && (brc == BSWRC_OK))
  {
    ...
    for (int x=0; x< no_of_intervals; x++)
    {
      ...
      InsertRowToMatrix(soap,ReturnData.returnDataRowsPerGroup[blockInd]);//End Row
      rs2.Next();
    }
  ...
  //csa_web_group_hourly_reply_log_d << LOG_TERMINATOR;

  return rc;
}


unsigned char GroupHourlyReply::GetNlaFieldFormat( char *arg_sql_format)
{
...
void   GroupHourlyReply::SetDateTodayAndTimeNow(EosOpCode_t op_code,
                                         Uint_t nla_int,
