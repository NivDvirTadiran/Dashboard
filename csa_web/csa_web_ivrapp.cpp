...
void IVRapp_tab::LoadList(void)
{
	...
	er_sprintf(sql_statement,"select ivr_app_name, ivr_app_id from ivr_apps");
	...
	else
	{
		...
		{ 			
			...
			{ 
           	if (!(rs.GetStringByName("ivr_app_name",Name))   ||
			    !(rs.GetUlongByName("ivr_app_id",(Ulong_t &)Id)))
				{
...
//--------------------------
string IVRapp_tab::GetName(unsigned int Id)
	{ string s = "";
...
//--------------------------
bool IVRapp_tab::isMonitor(void)
	{
		if(isMonitored)
			return true;
     isMonitored = true;
