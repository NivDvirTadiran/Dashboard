...
void IVRgrpport_tab::LoadList(void)
{
	...
	er_sprintf(sql_statement,"select ivr_grp_name, ivr_grp_id from ivr_groups");
	...
	else
	{
		...
		{ 			
			...
			{ 
              if (!(rs.GetStringByName("ivr_grp_name",Name))   ||
			!(rs.GetUlongByName("ivr_grp_id",(Ulong_t &)Id)))
				{
...
//--------------------------
string IVRgrpport_tab::GetName(unsigned int Id)
	{ string s = "";
...
bool IVRgrpport_tab::isMonitor(void)
	{
		if(isMonitored)
			return true;
     isMonitored = true;
